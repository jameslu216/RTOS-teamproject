...
{
	unsigned int ulDefaultIPAddress;
	unsigned int ulNetMask;
	unsigned int ulGatewayAddress;
	unsigned int ulDNSServerAddress;
	unsigned int ulBroadcastAddress;
...
struct xETH_HEADER
{
	...
	unsigned short usFrameType;              /* 12 + 2 = 14 */
}
...
struct xARP_HEADER
{
	unsigned short usHardwareType;				/*  0 +  2 =  2 */
	unsigned short usProtocolType;				/*  2 +  2 =  4 */
	unsigned char ucHardwareAddressLength;		/*  4 +  1 =  5 */
	unsigned char ucProtocolAddressLength;		/*  5 +  1 =  6 */
	unsigned short usOperation;					/*  6 +  2 =  8 */
...
struct xIP_HEADER
{
	unsigned char ucVersionHeaderLength;        /*  0 + 1 =  1 */
	unsigned char ucDifferentiatedServicesCode; /*  1 + 1 =  2 */
	unsigned short usLength;                    /*  2 + 2 =  4 */
	unsigned short usIdentification;            /*  4 + 2 =  6 */
	unsigned short usFragmentOffset;            /*  6 + 2 =  8 */
	unsigned char ucTimeToLive;                 /*  8 + 1 =  9 */
	unsigned char ucProtocol;                   /*  9 + 1 = 10 */
	unsigned short usHeaderChecksum;            /* 10 + 2 = 12 */
	unsigned int ulSourceIPAddress;           /* 12 + 4 = 16 */
	unsigned int ulDestinationIPAddress;      /* 16 + 4 = 20 */
}
#include "pack_struct_end.h"
...
struct xIGMP_HEADER
{
	unsigned char ucVersionType;     /* 0 + 1 = 1 */
	unsigned char ucMaxResponseTime; /* 1 + 1 = 2 */
	unsigned short usChecksum;       /* 2 + 2 = 4 */
	unsigned int usGroupAddress;   /* 4 + 4 = 8 */
}
#include "pack_struct_end.h"
...
struct xICMP_HEADER
{
	unsigned char ucTypeOfMessage;   /* 0 + 1 = 1 */
	unsigned char ucTypeOfService;   /* 1 + 1 = 2 */
	unsigned short usChecksum;       /* 2 + 2 = 4 */
	unsigned short usIdentifier;     /* 4 + 2 = 6 */
	unsigned short usSequenceNumber; /* 6 + 2 = 8 */
}
#include "pack_struct_end.h"
...
struct xUDP_HEADER
{
	unsigned short usSourcePort;      /* 0 + 2 = 2 */
	unsigned short usDestinationPort; /* 2 + 2 = 4 */
	unsigned short usLength;          /* 4 + 2 = 6 */
	unsigned short usChecksum;        /* 6 + 2 = 8 */
}
#include "pack_struct_end.h"
...
struct xTCP_HEADER
{
	unsigned short usSourcePort;		/* +  2 =  2 */
	unsigned short usDestinationPort;	/* +  2 =  4 */
	unsigned int ulSequenceNumber;	/* +  4 =  8 */
	unsigned int ulAckNr;   	 	/* +  4 = 12 */
	unsigned char  ucTcpOffset;		/* +  1 = 13 */
	unsigned char  ucTcpFlags;		/* +  1 = 14 */
	unsigned short usWindow;			/* +  2 = 15 */
	unsigned short usChecksum;		/* +  2 = 18 */
	unsigned short usUrgent;			/* +  2 = 20 */
#if ipconfigUSE_TCP == 1
	/* the option data is not a part of the TCP header */
	unsigned char  ucOptdata[ipSIZE_TCP_OPTIONS];		/* + 12 = 32 */
#endif
}
#include "pack_struct_end.h"
...
{
	unsigned int ulSourceAddress;
	unsigned int ulDestinationAddress;
	unsigned char ucZeros;
	unsigned char ucProtocol;
	unsigned short usUDPLength;
}
#include "pack_struct_end.h"
...
{
	unsigned char ucSocketOptions;
	unsigned char ucPadFor16BitAlignment;
	unsigned short usFragmentedPacketOffset;
	unsigned short usFragmentLength;
	unsigned short usPayloadChecksum;
}
#include "pack_struct_end.h"
...
extern const portBASE_TYPE xBufferAllocFixedSize;

/* Defined in FreeRTOS_Sockets.c */
#if ( ipconfigUSE_TCP == 1 )
	extern xList xBoundTcpSocketsList;
#endif

/* The local IP address is accessed from within xDefaultPartUDPPacketHeader,
rather than duplicated in its own variable. */
#define ipLOCAL_IP_ADDRESS_POINTER ( ( unsigned int * ) &( xDefaultPartUDPPacketHeader.ulWords[ 20 / sizeof(unsigned int) ] ) )
...
{ \
	( ( unsigned char* )( pxBase ) ) [ offsetof( xType, xField ) + 0 ] = ( unsigned char ) ( ( usValue ) >> 8 ); \
	( ( unsigned char* )( pxBase ) ) [ offsetof( xType, xField ) + 1 ] = ( unsigned char ) ( ( usValue ) & 0xff ); \
}

#define vSetField32( pxBase, xType, xField, ulValue ) \
{ \
	( (unsigned char*)( pxBase ) ) [ offsetof( xType, xField ) + 0 ] = ( unsigned char )   ( ( ulValue ) >> 24 ); \
	( (unsigned char*)( pxBase ) ) [ offsetof( xType, xField ) + 1 ] = ( unsigned char ) ( ( ( ulValue ) >> 16 ) & 0xff ); \
	( (unsigned char*)( pxBase ) ) [ offsetof( xType, xField ) + 2 ] = ( unsigned char ) ( ( ( ulValue ) >> 8 ) & 0xff ); \
	( (unsigned char*)( pxBase ) ) [ offsetof( xType, xField ) + 3 ] = ( unsigned char )   ( ( ulValue ) & 0xff ); \
}

#define vFlip_16( left, right ) \
	do { \
		unsigned short tmp = (left); \
		(left) = (right); \
		(right) = tmp; \
	} while (0)
...
#ifndef ARRAY_SIZE
	#define ARRAY_SIZE(x)	(portBASE_TYPE)(sizeof(x)/sizeof(x)[0])
#endif
...
portBASE_TYPE xProcessReceivedUDPPacket( NetworkBufferDescriptor_t *pxNetworkBuffer, unsigned short usPort );
...
portBASE_TYPE xIPIsNetworkTaskReady( void );
	...
	{
		unsigned int ulRemoteIP;		/* IP address of remote machine */
		unsigned short usRemotePort;		/* Port on remote machine */
		struct {
		...
		unsigned int ulHighestRxAllowed;
								/* The highest sequence number that we can receive at any moment */
		unsigned short usTimeout;		/* Time (in ticks) after which this socket needs attention */
		unsigned short usCurMSS;		/* Current Maximum Segment Size */
		unsigned short usInitMSS;		/* Initial maximum segment Size */
		unsigned short usChildCount;	/* In case of a listening socket: number of connections on this port number */
		unsigned short usBacklog;		/* In case of a listening socket: maximum number of concurrent connections on this port number */
		unsigned char ucRepCount;		/* Send repeat count, for retransmissions
								 * This counter is separate from the xmitCount in the
								 * TCP win segments */
		unsigned char ucTcpState;		/* TCP state: see eTCP_STATE */
		struct XSOCKET *pxPeerSocket;	/* for server socket: child, for child socket: parent */
		#if( ipconfigTCP_KEEP_ALIVE == 1 )
			unsigned char ucKeepRepCount;
			portTickType xLastAliveTime;
		#endif /* ipconfigTCP_KEEP_ALIVE */
		#if( ipconfigTCP_HANG_PROTECTION == 1 )
			portTickType xLastActTime;
		...
		unsigned char fillPacket[ ipconfigPACKET_FILLER_SIZE ];
		unsigned char lastPacket[ sizeof( TCPPacket_t ) ];
		unsigned char tcpflags;		/* TCP flags */
			...
			FOnTcpReceive pHndReceive;	/*
										 ...
										 */
			FOnTcpSent pHndSent;
			FOnConnected pHndConnected;	/* Actually type: typedef void (* FOnConnected) (Socket_t xSocket, portBASE_TYPE ulConnected ); */
		#endif /* ipconfigUSE_CALLBACKS */
		unsigned int wnd;			/* Current Window size advertized by peer */
		unsigned int ulRxCurWinSize;	/* Constantly changing: this is the current size available for data reception */
...
{
	xList xWaitingPacketsList;	/* Incoming packets */
	#if( ipconfigUDP_MAX_RX_PACKETS > 0 )
		unsigned portBASE_TYPE xMaxPackets; /* Protection: limits the number of packets buffered per socket */
...
typedef struct XSOCKET
{
	...
	portTickType xReceiveBlockTime; /* if recv[to] is called while no data is available, wait this amount of time. Unit in clock-ticks */
	portTickType xSendBlockTime; /* if send[to] is called while there is not enough space to send, wait this amount of time. Unit in clock-ticks */

	unsigned short usLocPort;		/* Local port on this machine */
	unsigned char ucSocketOptions;
	unsigned char ucProtocol; /* choice of FREERTOS_IPPROTO_UDP/TCP */
 ...
 */
int lTCPAddRxdata(FreeRTOS_Socket_t *pxSocket, portBASE_TYPE ulOffset, const unsigned char *pcData, unsigned int ulCount);
 ...
 */
static portINLINE unsigned int ulChar2u32 (const unsigned char *apChr)
{
	return  ( ( ( unsigned int )apChr[0] ) << 24) |
			( ( ( unsigned int )apChr[1] ) << 16) |
			( ( ( unsigned int )apChr[2] ) << 8) |
			( ( ( unsigned int )apChr[3] ) );
}

static portINLINE unsigned short usChar2u16 (const unsigned char *apChr)
{
	return ( unsigned short )
			( ( ( ( unsigned int )apChr[0] ) << 8) |
			  ( ( ( unsigned int )apChr[1] ) ) );
}
...
static portINLINE unsigned portBASE_TYPE uxGetRxEventCount( void )
{
	extern volatile unsigned portBASE_TYPE uxRxEventCount;
...
typedef struct xSOCKET_SET
{
	...
	portBASE_TYPE bApiCalled;	/* True if the API was calling  the private vSocketSelect */
