 ...
 */
static portBASE_TYPE prvTCPSocketIsActive( portBASE_TYPE lStatus );
 ...
 */
static portBASE_TYPE prvTCPSendRepeated( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t **ppxNetworkBuffer );
...
static void prvTCPReturnPacket( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t *pxNetworkBuffer,
	unsigned int ulLen, portBASE_TYPE xReleaseAfterSend );
 ...
 * packet.
 */
static portBASE_TYPE prvTCPPrepareConnect( FreeRTOS_Socket_t *pxSocket );

#if( ipconfigHAS_DEBUG_PRINTF != 0 )
	/*
	 * For logging and debugging: make a string showing the TCP flags.
	 */
	static const char *prvTCPFlagMeaning( UportBASE_TYPE xFlags);
 ...
 * 'eCONNECT_SYN'.
 */
static portBASE_TYPE prvSetSynAckOptions( FreeRTOS_Socket_t *pxSocket, TCPPacket_t * pxTCPPacket );
 ...
 */
static int prvTCPPrepareSend( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t **ppxNetworkBuffer, portBASE_TYPE uxOptionsLength );

/*
 * Calculate when this socket needs to be checked to do (re-)transmissions.
 */
static portTickType prvTCPNextTimeout( FreeRTOS_Socket_t *pxSocket );
 ...
 */
static portBASE_TYPE prvTCPHandleFin( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t *pxNetworkBuffer );

#if(	ipconfigUSE_TCP_TIMESTAMPS == 1 )
	static portBASE_TYPE prvTCPSetTimeStamp( portBASE_TYPE lOffset, FreeRTOS_Socket_t *pxSocket, TCPHeader_t *pxTCPHeader );
#endif
 ...
 * return its length.
 */
static portBASE_TYPE prvCheckRxData( NetworkBufferDescriptor_t *pxNetworkBuffer, unsigned char **ppucRecvData );
 ...
 */
static portBASE_TYPE prvStoreRxData( FreeRTOS_Socket_t *pxSocket, unsigned char *pucRecvData,
	NetworkBufferDescriptor_t *pxNetworkBuffer, unsigned int ulReceiveLength );

/*
 * Set the TCP options (if any) for the outgoing packet.
 */
static portBASE_TYPE prvSetOptions( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t *pxNetworkBuffer );

/*
 * Called from prvTCPHandleState() as long as the TCP status is eSYN_RECEIVED to
 * eCONNECT_SYN.
 */
static portBASE_TYPE prvHandleSynReceived( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t **ppxNetworkBuffer,
	unsigned int ulReceiveLength, portBASE_TYPE xOptionsLength );

/*
 * Called from prvTCPHandleState() as long as the TCP status is eESTABLISHED.
 */
static portBASE_TYPE prvHandleEstablished( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t **ppxNetworkBuffer,
	unsigned int ulReceiveLength, portBASE_TYPE xOptionsLength );
 ...
 */
static portBASE_TYPE prvSendData( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t **ppxNetworkBuffer,
	unsigned int ulReceiveLength, portBASE_TYPE xSendLength );

/*
 * The heart of all: check incoming packet for valid data and acks and do what
 * is necessary in each state.
 */
static portBASE_TYPE prvTCPHandleState( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t **ppxNetworkBuffer );

/*
 * Reply to a peer with the RST flag on, in case a packet can not be handled.
 */
static portBASE_TYPE prvTCPSendReset( NetworkBufferDescriptor_t *pxNetworkBuffer );
 ...
 */
static portBASE_TYPE prvTCPSocketCopy( FreeRTOS_Socket_t *pxNewSocket, FreeRTOS_Socket_t *pxSocket );
 ...
 */
	...
	static portBASE_TYPE prvTCPStatusAgeCheck( FreeRTOS_Socket_t *pxSocket );
#endif
/*-----------------------------------------------------------*/
...
unsigned int ulNextInitialSequenceNumber = 0;
...
/*-----------------------------------------------------------*/
	...
	portBASE_TYPE xResult;
		...
		if( xResult != pdFALSE )
		{
			...
			portTickType xAge = xTaskGetTickCount( ) - pxSocket->u.xTcp.xLastActTime;
			...
			if( xAge > ( ipconfigTCP_HANG_PROTECTION_TIME * configTICK_RATE_HZ ) )
			{
				#if( ipconfigHAS_DEBUG_PRINTF == 1 )
				{
					FreeRTOS_debug_printf( ( "Inactive socket closed: port %u rem %lxip:%u status %s\n",
						...
						FreeRTOS_GetTCPStateName( ( UportBASE_TYPE ) pxSocket->u.xTcp.ucTcpState ) ) );
				}
 ...
 */
portBASE_TYPE xTCPSocketCheck( FreeRTOS_Socket_t *pxSocket )
{
portBASE_TYPE xResult = 0;
portBASE_TYPE xReady = pdFALSE;
...
static int prvTCPSendPacket( FreeRTOS_Socket_t *pxSocket )
{
portBASE_TYPE lResult = 0;
portBASE_TYPE xOptionsLength;
...
NetworkBufferDescriptor_t *pxNetworkBuffer;
	...
	else
	{
		...
		else if( ( pxSocket->u.xTcp.bits.bConnPrepared ) || ( prvTCPPrepareConnect( pxSocket ) == pdTRUE ) )
		{
			...
			pxTCPPacket->xTCPHeader.ucTcpOffset = ( unsigned char )( ( ipSIZE_OF_TCP_HEADER + xOptionsLength ) << 2 );
			...
			prvTCPReturnPacket( pxSocket, NULL, ( unsigned int ) lResult, pdFALSE );
		}
	}
...
static portBASE_TYPE prvTCPSendRepeated( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t **ppxNetworkBuffer )
{
portBASE_TYPE lIndex;
portBASE_TYPE lResult = 0;
portBASE_TYPE xOptionsLength = 0;
portBASE_TYPE xSendLength;
	...
	for( lIndex = 0; ( lIndex < SEND_REPEATED_COUNT ) && ( uxGetRxEventCount() == 0 ); lIndex++ )
	{
		...
		prvTCPReturnPacket (pxSocket, *ppxNetworkBuffer, ( unsigned int ) xSendLength, ipconfigZERO_COPY_TX_DRIVER );
 ...
 * the data to the NIC.
 */
static void prvTCPReturnPacket( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t *pxNetworkBuffer, unsigned int ulLen, portBASE_TYPE xReleaseAfterSend )
{
TCPPacket_t * pxTCPPacket;
IPHeader_t *pxIPHeader;
EthernetHeader_t *pxEthernetHeader;
unsigned int ulFrontSpace, ulSpace;
	...
	if( pxNetworkBuffer != NULL )
	{
		...
		if( pxSocket != NULL )
		{
			...
			if( pxSocket->u.xTcp.rxStream != NULL )
			{
				...
				available. */
				ulFrontSpace = ( unsigned int ) uxStreamBufferFrontSpace( pxSocket->u.xTcp.rxStream );
			}
			else
			{
				/* No RX stream has been created, the full stream size is
				available. */
				ulFrontSpace = ( unsigned int ) pxSocket->u.xTcp.uxRxStreamSize;
			}
			...
			pxTCPPacket->xTCPHeader.usWindow = FreeRTOS_htons( ( unsigned short ) ulSpace );
			...
			#if( ipconfigHAS_DEBUG_PRINTF != 0 )
			{
				if( ipconfigTCP_MAY_LOG_PORT( pxSocket->usLocPort ) != pdFALSE )
				{
					...
					{
						int lFrontSpace = pxSocket->u.xTcp.rxStream ? uxStreamBufferFrontSpace( pxSocket->u.xTcp.rxStream ) : 0;
						...
						pxSocket->u.xTcp.bits.bLowWater ? "STOP" : "GO ",
							...
							(int) ( pxTcpWindow->rx.ulHighestSequenceNumber - pxTcpWindow->rx.ulCurrentSequenceNumber ) ) );
					}
				}
			}
			#endif /* ipconfigHAS_DEBUG_PRINTF != 0 */
			...
			#endif
			{
				...
				if( ( pxTCPPacket->xTCPHeader.ucTcpFlags & ipTCP_FLAG_FIN ) != 0 )
				{
					...
					retransmitted. */
					unsigned int ulDataLen = ( unsigned int ) ( ulLen - ( ipSIZE_OF_TCP_HEADER + ipSIZE_OF_IP_HEADER ) );
					if( ( pxTcpWindow->ulOurSequenceNumber + ulDataLen ) != pxTcpWindow->tx.ulFINSequenceNumber )
					{
						pxTCPPacket->xTCPHeader.ucTcpFlags &= ( ( unsigned char ) ~ipTCP_FLAG_FIN );
		...
		#if( ipconfigDRIVER_INCLUDED_TX_IP_CHECKSUM == 0 )
		{
			...
			usGenerateProtocolChecksum( (unsigned char*)pxTCPPacket, pdTRUE );
		...
		else if( ( pucPtr[0] == TCP_OPT_MSS ) && ( pucPtr[1] == TCP_OPT_MSS_LEN ) )
		{
			...
			if( pxSocket->u.xTcp.usInitMSS > xNewMSS )
			{
				...
				if( pxTcpWindow && pxSocket->u.xTcp.usCurMSS > xNewMSS )
				{
					...
					pxSocket->u.xTcp.usCurMSS = ( unsigned short ) xNewMSS;
				}
				pxTcpWindow->xSize.ulRxWindowLength = ( ( unsigned int ) xNewMSS ) * ( pxTcpWindow->xSize.ulRxWindowLength / ( ( unsigned int ) xNewMSS ) );
				pxTcpWindow->usMSSInit = ( unsigned short ) xNewMSS;
				pxTcpWindow->usMSS = ( unsigned short ) xNewMSS;
				pxSocket->u.xTcp.usInitMSS = ( unsigned short ) xNewMSS;
				pxSocket->u.xTcp.usCurMSS = ( unsigned short ) xNewMSS;
			}
		...
		else
		{
			...
			#if( ipconfigUSE_TCP_WIN == 1 )
			{
				...
				if( pucPtr[0] == TCP_OPT_SACK_A )
				{
					...
					while( len >= 8 )
					{
					unsigned int ulFirst = ulChar2u32( pucPtr );
					unsigned int ulLast  = ulChar2u32( pucPtr + 4 );
					unsigned int ulCount = ulTCPWindowTxSack( &pxSocket->u.xTcp.xTcpWindow, ulFirst, ulLast );
...
unsigned short usMSS = pxSocket->u.xTcp.usInitMSS;
#if	ipconfigUSE_TCP_WIN == 1
portBASE_TYPE xOptionsLength;
#endif
	...
	pxTCPHeader->ucOptdata[2] = ( unsigned char ) ( usMSS >> 8 );
	pxTCPHeader->ucOptdata[3] = ( unsigned char ) ( usMSS & 0xff );
 ...
 * FreeRTOS_select()
 */
void vTCPStateChange( FreeRTOS_Socket_t *pxSocket, portBASE_TYPE xTcpState )
{
FreeRTOS_Socket_t *xParent;
portBASE_TYPE bBefore = NOW_CONNECTED( pxSocket->u.xTcp.ucTcpState );	/* Was it connected ? */
portBASE_TYPE bAfter  = NOW_CONNECTED( xTcpState );					/* Is it connected now ? */
#if( ipconfigHAS_DEBUG_PRINTF != 0 )
	portBASE_TYPE xPreviousState = pxSocket->u.xTcp.ucTcpState;
#endif
#if( ipconfigUSE_CALLBACKS == 1 )
	...
	pxSocket->u.xTcp.ucTcpState = ( unsigned char ) xTcpState;
	...
	if( ( xTCPWindowLoggingLevel >= 0 ) && ( ipconfigTCP_MAY_LOG_PORT( pxSocket->usLocPort ) != pdFALSE ) )
		FreeRTOS_debug_printf( ( "Socket %d -> %lxip:%u State %s->%s\n",
			...
			pxSocket->u.xTcp.ulRemoteIP,
			pxSocket->u.xTcp.usRemotePort,
			FreeRTOS_GetTCPStateName( ( UportBASE_TYPE ) xPreviousState ),
			FreeRTOS_GetTCPStateName( ( UportBASE_TYPE ) xTcpState ) ) );
	}
	#endif /* ipconfigHAS_DEBUG_PRINTF */
...
static NetworkBufferDescriptor_t *prvTCPBufferResize( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t *pxNetworkBuffer,
	int lDataLen, portBASE_TYPE xOptionsLength )
{
NetworkBufferDescriptor_t *pxReturn;
int lNeeded;
portBASE_TYPE xResize;
	...
	else
	{
		...
		grow. */
		lNeeded = FreeRTOS_max_int32( ( int ) sizeof( pxSocket->u.xTcp.lastPacket ),
	...
	if( xResize != pdFALSE )
	{
		...
		here. */
		pxReturn = pxGetNetworkBufferWithDescriptor( ( unsigned int ) lNeeded, 0 );
 ...
 */
static int prvTCPPrepareSend( FreeRTOS_Socket_t *pxSocket, NetworkBufferDescriptor_t **ppxNetworkBuffer, portBASE_TYPE xOptionsLength )
{
int lDataLen;
unsigned char *pucEthernetBuffer, *pucSendData;
TCPPacket_t *pxTCPPacket;
int lOffset;
unsigned int ulDataGot, ulDistance;
TCPWindow_t *pxTcpWindow;
...
int lStreamPos;
	...
	if( pxSocket->u.xTcp.txStream != NULL )
	{
		...
		{
			lDataLen = ( int ) ulTCPWindowTxGet( pxTcpWindow, pxSocket->u.xTcp.wnd, &lStreamPos );
		}
		...
		if( lDataLen > 0 )
		{
			...
			if( pxNewBuffer != NULL )
			{
				*ppxNetworkBuffer = pxNewBuffer;
				...
				ulDataGot = ( unsigned int ) uxStreamBufferGet( pxSocket->u.xTcp.txStream, ( size_t ) lOffset, pucSendData, ( size_t ) lDataLen, pdTRUE );

				#if( ipconfigHAS_DEBUG_PRINTF != 0 )
				{
					if( ulDataGot != ( unsigned int ) lDataLen )
					{
				...
				{
					ulDistance = ( unsigned int ) uxStreamBufferDistance( pxSocket->u.xTcp.txStream, ( size_t ) lStreamPos, pxSocket->u.xTcp.txStream->uxHead );
					...
					if( ulDistance == ulDataGot )
					{
						...
						pxTcpWindow->tx.ulFINSequenceNumber = pxTcpWindow->ulOurSequenceNumber + ( unsigned int ) lDataLen;
	...
	if( ( lDataLen >= 0 ) && ( pxSocket->u.xTcp.ucTcpState == eESTABLISHED ) )
	{
		...
		#if( ipconfigTCP_KEEP_ALIVE != 0 )
		{
			...
			if( ( lDataLen == 0 ) && ( pxSocket->u.xTcp.bits.bWinChange == 0 ) )
			{
				...
				portTickType xAge = xTaskGetTickCount( ) - pxSocket->u.xTcp.xLastAliveTime;
				portTickType xMax;
				...
				if( xAge > xMax )
				{
					...
					pxSocket->u.xTcp.usTimeout = ( ( unsigned short ) pdMS_TO_TICKS( 2500 ) );
					pxSocket->u.xTcp.ucKeepRepCount++;
				}
			}
		}
	...
	if( ( lDataLen > 0 ) ||
		...
		( pxSocket->u.xTcp.bits.bSendKeepAlive != pdFALSE ) )
	{
		pxTCPPacket->xTCPHeader.ucTcpFlags &= ( ( unsigned char ) ~ipTCP_FLAG_PSH );
		pxTCPPacket->xTCPHeader.ucTcpOffset = ( unsigned char )( ( ipSIZE_OF_TCP_HEADER + xOptionsLength ) << 2 );

		pxTCPPacket->xTCPHeader.ucTcpFlags |= ( unsigned char ) ipTCP_FLAG_ACK;
		...
		lDataLen += ( ipSIZE_OF_IP_HEADER + ipSIZE_OF_TCP_HEADER + ( ( int ) xOptionsLength ) );
	}
...
portTickType ulDelayMs = 20000;
	...
	if( pxSocket->u.xTcp.ucTcpState == eCONNECT_SYN )
	{
		...
		pxSocket->u.xTcp.usTimeout = ( unsigned short )pdMS_TO_TICKS( ulDelayMs );
	}
	else if( pxSocket->u.xTcp.usTimeout == 0 )
	{
		/* Let the sliding window mechanism decide what time-out is appropriate. */
		portBASE_TYPE xResult = xTCPWindowTxHasData( &pxSocket->u.xTcp.xTcpWindow, pxSocket->u.xTcp.wnd, &ulDelayMs );
		if( ulDelayMs == 0 )
		{
...
int lCount, lLength;
	...
	if( lLength > 0 )
	{
		...
		lCount = lTCPWindowTxAdd(	&pxSocket->u.xTcp.xTcpWindow,
								...
								( int ) pxSocket->u.xTcp.txStream->uxMid,
								( int ) pxSocket->u.xTcp.txStream->LENGTH );
...
TCPHeader_t *pxTCPHeader = &pxTCPPacket->xTCPHeader;
unsigned char ucTcpFlags = pxTCPHeader->ucTcpFlags;
TCPWindow_t *pxTcpWindow = &pxSocket->u.xTcp.xTcpWindow;
portBASE_TYPE xSendLength = 0;
unsigned int ulAckNr = FreeRTOS_ntohl( pxTCPHeader->ulAckNr );
	...
	pxTCPHeader->ucTcpOffset = ( unsigned char ) ( ( ipSIZE_OF_TCP_HEADER + pxTcpWindow->ucOptionLength ) << 2 );
...
/*-----------------------------------------------------------*/
	...
	static portBASE_TYPE prvTCPSetTimeStamp( portBASE_TYPE lOffset, FreeRTOS_Socket_t *pxSocket, TCPHeader_t *pxTCPHeader )
	{
	unsigned int ulTimes[2];
	unsigned char *ucOptdata = &( pxTCPHeader->ucOptdata[ lOffset ] );
...
TCPPacket_t *pxTCPPacket = ( TCPPacket_t * ) ( pxNetworkBuffer->pucEthernetBuffer );
TCPHeader_t *pxTCPHeader = &( pxTCPPacket->xTCPHeader );
int lLength, lTCPHeaderLength, lReceiveLength, lUrgentLength;
	...
	(offset >> 4) * 4. */
    lTCPHeaderLength = ( ( portBASE_TYPE ) ( pxTCPHeader->ucTcpOffset & VALID_BITS_IN_TCP_OFFSET_BYTE ) ) >> 2;
	...
	*ppucRecvData = pxNetworkBuffer->pucEthernetBuffer + ipSIZE_OF_ETH_HEADER + ipSIZE_OF_IP_HEADER + lTCPHeaderLength;
	...
	lReceiveLength = ( ( int ) pxNetworkBuffer->xDataLength ) - ipSIZE_OF_ETH_HEADER;
...
TCPHeader_t *pxTCPHeader = &pxTCPPacket->xTCPHeader;
TCPWindow_t *pxTcpWindow = &pxSocket->u.xTcp.xTcpWindow;
unsigned int ulSequenceNumber, ulSpace;
int lOffset, lStored;
portBASE_TYPE xResult = 0;
	...
	if( ( ulReceiveLength > 0 ) && ( pxSocket->u.xTcp.ucTcpState >= eSYN_RECEIVED ) )
	{
		...
		{
			ulSpace = ( unsigned int )uxStreamBufferGetSpace ( pxSocket->u.xTcp.rxStream );
		}
		else
		{
			ulSpace = ( unsigned int )pxSocket->u.xTcp.uxRxStreamSize;
		}
		...
		if( lOffset >= 0 )
		{
			...
			if( lStored != ( int ) ulReceiveLength )
			{
...
TCPHeader_t *pxTCPHeader = &pxTCPPacket->xTCPHeader;
TCPWindow_t *pxTcpWindow = &pxSocket->u.xTcp.xTcpWindow;
portBASE_TYPE xOptionsLength = pxTcpWindow->ucOptionLength;
		...
		if( xOptionsLength )
		{
			...
			pxTCPHeader->ucTcpOffset = ( unsigned char )( ( ipSIZE_OF_TCP_HEADER + xOptionsLength ) << 2 );
		}
		else
	#endif	/* ipconfigUSE_TCP_WIN */
	if( ( pxSocket->u.xTcp.ucTcpState >= eESTABLISHED ) && ( pxSocket->u.xTcp.bits.bMssChange != 0 ) )
	{
		...
		pxTCPHeader->ucOptdata[ 0 ] = TCP_OPT_MSS;
		pxTCPHeader->ucOptdata[ 1 ] = TCP_OPT_MSS_LEN;
		pxTCPHeader->ucOptdata[ 2 ] = ( unsigned char ) ( ( pxSocket->u.xTcp.usCurMSS ) >> 8 );
		pxTCPHeader->ucOptdata[ 3 ] = ( unsigned char ) ( ( pxSocket->u.xTcp.usCurMSS ) & 0xff );
		xOptionsLength = 4;
		pxTCPHeader->ucTcpOffset = ( unsigned char )( ( ipSIZE_OF_TCP_HEADER + xOptionsLength ) << 2 );
	}
...
TCPWindow_t *pxTcpWindow = &pxSocket->u.xTcp.xTcpWindow;
unsigned char ucTcpFlags = pxTCPHeader->ucTcpFlags;
unsigned int ulSequenceNumber = FreeRTOS_ntohl( pxTCPHeader->ulSequenceNumber );
portBASE_TYPE xSendLength = 0;

	/* Either expect a ACK or a SYN+ACK. */
	unsigned short usExpect = ipTCP_FLAG_ACK;
	if( pxSocket->u.xTcp.ucTcpState == eCONNECT_SYN )
	{
...
TCPWindow_t *pxTcpWindow = &pxSocket->u.xTcp.xTcpWindow;
unsigned char ucTcpFlags = pxTCPHeader->ucTcpFlags;
unsigned int ulSequenceNumber = FreeRTOS_ntohl( pxTCPHeader->ulSequenceNumber ), ulCount;
portBASE_TYPE xSendLength = 0, xMayClose = pdFALSE, bRxComplete, bTxDone;
int lDistance, lSendResult;
	...
	if( ( pxSocket->u.xTcp.bits.bFinAccepted != 0 ) || ( ( ucTcpFlags & ipTCP_FLAG_FIN ) != 0 ) )
	{
		...
		if( pxSocket->u.xTcp.bits.bFinSent == 0 )
		{
			...
			{
				lDistance = ( int ) ( ulSequenceNumber + ulReceiveLength - pxTcpWindow->rx.ulCurrentSequenceNumber );
...
unsigned int ulFrontSpace;
int lRxSpace;
#if( ipconfigTCP_ACK_EARLIER_PACKET == 0 )
	const int lMinLength = 0;
#else
	int lMinLength;
#endif
	...
	lRxSpace = (int)( pxSocket->u.xTcp.ulHighestRxAllowed - pxTcpWindow->rx.ulCurrentSequenceNumber );

	#if( ipconfigTCP_ACK_EARLIER_PACKET != 0 )
	{
		lMinLength = ( ( int ) 2 ) * ( ( int ) pxSocket->u.xTcp.usCurMSS );
	}
	...
	#if ipconfigUSE_TCP_WIN == 1
	{
			...
			( pxTCPHeader->ucTcpFlags == ipTCP_FLAG_ACK ) )		/* There are no other flags than an ACK. */
		{
			...
			if( ( ulReceiveLength < ( unsigned int ) pxSocket->u.xTcp.usCurMSS ) ||	/* Received a small message. */
				( lRxSpace < ( int ) ( 2U * pxSocket->u.xTcp.usCurMSS ) ) )	/* There are less than 2 x MSS space in the Rx buffer. */
			{
				pxSocket->u.xTcp.usTimeout = ( unsigned short ) pdMS_TO_TICKS( DELAYED_ACK_SHORT_DELAY_MS );
			}
			else
			{
				...
				pxSocket->u.xTcp.usTimeout = ( unsigned short ) pdMS_TO_TICKS( DELAYED_ACK_LONGER_DELAY_MS );
			}
	...
	if( xSendLength != 0 )
	{
		...
		prvTCPReturnPacket( pxSocket, *ppxNetworkBuffer, ( unsigned int ) xSendLength, pdFALSE );
	}
...
TCPHeader_t *pxTCPHeader = &( pxTCPPacket->xTCPHeader );
portBASE_TYPE xSendLength = 0;
unsigned int ulReceiveLength;	/* Number of bytes contained in the TCP message. */
...
TCPWindow_t *pxTcpWindow = &( pxSocket->u.xTcp.xTcpWindow );
	...
	ulReceiveLength = ( unsigned int ) prvCheckRxData( *ppxNetworkBuffer, &pucRecvData );
	...
	this connection. */
	if( ( ( int ) ( ulSequenceNumber + ulReceiveLength - pxTcpWindow->rx.ulHighestSequenceNumber ) ) > 0 )
	{
...
TCPPacket_t *pxTCPPacket = ( TCPPacket_t * ) ( pxNetworkBuffer->pucEthernetBuffer );
const portBASE_TYPE xSendLength = ipSIZE_OF_IP_HEADER + ipSIZE_OF_TCP_HEADER + 0;	/* plus 0 options. */
	...
	prvTCPReturnPacket( NULL, pxNetworkBuffer, ( unsigned int ) xSendLength, pdFALSE );
...
static void prvSocketSetMSS( FreeRTOS_Socket_t *pxSocket )
{
unsigned int ulMSS = ipconfigTCP_MSS;
	...
	pxSocket->u.xTcp.usInitMSS = pxSocket->u.xTcp.usCurMSS = ( unsigned short ) ulMSS;
}
/*-----------------------------------------------------------*/
 ...
 *			xNetworkInterfaceOutput()	// Sends data to the NIC
*/
portBASE_TYPE xProcessReceivedTCPPacket( NetworkBufferDescriptor_t *pxNetworkBuffer )
{
FreeRTOS_Socket_t *pxSocket;
TCPPacket_t * pxTCPPacket = ( TCPPacket_t * ) ( pxNetworkBuffer->pucEthernetBuffer );
unsigned short ucTcpFlags = pxTCPPacket->xTCPHeader.ucTcpFlags;
unsigned int ulLocalIP = FreeRTOS_htonl( pxTCPPacket->xIPHeader.ulDestinationIPAddress );
unsigned short xLocalPort = FreeRTOS_htons( pxTCPPacket->xTCPHeader.usDestinationPort );
unsigned int ulRemoteIP = FreeRTOS_htonl( pxTCPPacket->xIPHeader.ulSourceIPAddress );
unsigned short xRemotePort = FreeRTOS_htons( pxTCPPacket->xTCPHeader.usSourcePort );
		...
		if( pxSocket->u.xTcp.ucTcpState == eTCP_LISTEN )
		{
			...
			if( ( ucTcpFlags & ipTCP_FLAG_CTRL ) != ipTCP_FLAG_SYN )
			{
				...
				FreeRTOS_debug_printf( ( "TCP: Server can't handle flags: %s from %lxip:%u to port %u\n",
					prvTCPFlagMeaning( ( UportBASE_TYPE ) ucTcpFlags ), ulRemoteIP, xRemotePort, xLocalPort ) );
				}
				#endif /* ipconfigHAS_DEBUG_PRINTF */
...
/*-----------------------------------------------------------*/
	...
	const char *FreeRTOS_GetTCPStateName( UportBASE_TYPE ulState )
	{
 ...
 */
portBASE_TYPE xTCPCheckNewClient( FreeRTOS_Socket_t *pxSocket )
{
portTickType xLocalPort = FreeRTOS_htons( pxSocket->usLocPort );
