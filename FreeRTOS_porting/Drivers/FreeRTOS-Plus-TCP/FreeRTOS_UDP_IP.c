...
#define ipIP_VERSION_AND_HEADER_LENGTH_BYTE ( ( unsigned char ) 0x45 )
...
unsigned int ulIPAddress = pxNetworkBuffer->ulIPAddress;
	...
	if( eReturned != eCantSendPacket )
	{
		if( eReturned == eARPCacheHit )
		{
			...
			UDPHeader_t *pxUDPHeader;
				...
				pxUDPHeader->usLength = ( unsigned short ) ( pxNetworkBuffer->xDataLength + sizeof( UDPHeader_t ) );
			...
			*/
			...
			if( pxNetworkBuffer->usPort == ipPACKET_CONTAINS_ICMP_DATA )
			{
				...
				pxIPHeader->usLength = ( unsigned short ) ( pxNetworkBuffer->xDataLength + sizeof( IPHeader_t ) );
			}
			else
		#endif /* ipconfigSUPPORT_OUTGOING_PINGS */
			{
				pxIPHeader->usLength = ( unsigned short ) ( pxNetworkBuffer->xDataLength + sizeof( IPHeader_t ) + sizeof( UDPHeader_t ) );
			}
...
UDPPacket_t *pxUDPPacket = (UDPPacket_t *) pxNetworkBuffer->pucEthernetBuffer;
	...
	else
	{
		...
		be for this node. */
			...
			{
				vARPRefreshCacheEntry( &( pxUDPPacket->xEthernetHeader.xSourceAddress ), pxUDPPacket->xIPHeader.ulSourceIPAddress );
				xReturn = ( portBASE_TYPE )ulDNSHandlePacket( pxNetworkBuffer );
			}
			else
		#endif
			...
			{
				vARPRefreshCacheEntry( &( pxUDPPacket->xEthernetHeader.xSourceAddress ), pxUDPPacket->xIPHeader.ulSourceIPAddress );
				xReturn = ( portBASE_TYPE )ulNBNSHandlePacket( pxNetworkBuffer );
			}
			else
